...
void Button::buttonHandlerTask(void *ptr)
{
    ...
    const TickType_t xFrequency = pdMS_TO_TICKS(5); // 5ms non-blocking delay
...
void Button::enterSleep()
{
    ...
    esp_sleep_enable_timer_wakeup(3 * 1000000ULL); // 3 second timer instead of blocking delay
    esp_light_sleep_start();                       // Light sleep for 3 seconds, then continue to deep sleep
